...
export default function Sidebar({ options }: { options: Record<string, any>[] }) {
    ...
    return (
        <>
            <BsList className={`hover:scale-110 text-2xl absolute top-4 right-2 text-white cursor-pointer z-50 ${showSidebar && "hidden"}`} onClick={() => setShowSidebar(!showSidebar)} />
            {
                showSidebar && (
                    <div className="bg-gray-950 text-white flex flex-col overflow-y-scroll h-screen md:overflow-y-hidden md:h-auto md:min-h-screen fixed left-0 top-0 w-full lg:relative lg:min-w-[300px] lg:w-1/5 select-none">
                        <BsX className="text-2xl text-white absolute top-4 right-2 hover:scale-110 cursor-pointer" onClick={() => setShowSidebar(false)} />
                        ...
                        <ul className="w-full flex flex-col items-center justify-center">
                            {
                                options && options.map((o: Record<string, any>, i: number) => {
                                    if (o.link) {
                                        return <Link onClick={() => setShowSidebar(false)} prefetch={false} key={i} href={o.link} className="relative w-full py-4 bg-gray-800 hover:bg-gray-700 flex items-center justify-between p-2 border-b">
                                            {
                                                o.icon && (
                                                    o.icon
                                                )
                                    ...
                                    if (o.dropdown) {
                                        return <Dropdown key={i} name={o.name} dropdown={o.dropdown} notify={o.notify} icon={o.icon} setShowSidebar={setShowSidebar} />
                                    }
...
export function Dropdown({ name, dropdown, notify, icon, setShowSidebar }: { name: string, dropdown: { name: string, link: string, notify?: number }[], notify?: number, icon?: React.ReactNode, setShowSidebar: Function }) {
            ...
            </span>
            {
                showDropdown && (
                    <div className="flex flex-col w-full">
                        {
                            dropdown.map((d, i) => {
                                return <Link onClick={() => setShowSidebar(false)} prefetch={false} key={i} href={d.link} className="relative w-full py-4 bg-blue-950 hover:bg-blue-900 text-center border-b">
                                    <li>{d.name}</li>
                                    {
                                        (Number(d.notify) > 0) && (
