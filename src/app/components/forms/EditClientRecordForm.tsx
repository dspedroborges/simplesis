...
export default function EditClientRecordForm({ existingRecord }: { existingRecord: string }) {
    ...
    return (
        <>
            <TopBar title="Editar ficha para cliente" />
            {
                formState.error ? (
            ...
            <form defaultValue={formState.message} action={action} ref={formRef} className="mt-8 p-2 w-full md:mx-auto md:w-1/2 flex flex-col gap-4">
                <div>
                    ...
                    <textarea name="record" id="record" className="w-full min-h-[500px] border p-2" defaultValue={existingRecord}></textarea>
                </div>
